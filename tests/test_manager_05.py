...
def worker_message_generator(mock_sendall, tmp_path):
    ...
    # Wait for Manager to create shared directory
    #
    ...
    tmpdir = ""
    for tmpdir in (
        utils.wait_for_exists_glob(f"{tmp_path}/mapreduce-shared-*")
    ):
        yield None
    ...
    # Wait for Manager to create job 0 directory
    tmpdir_job0 = tmpdir / "job-00000"
    utils.wait_for_exists(tmpdir_job0)
...
def worker_heartbeat_generator():
    ...
    # Worker 3002 sends a single heartbeat. It's OK if Worker 3002 hasn't
    # registered yet, because the Manager should ignore heartbeats from
...
def test_dead_worker(mocker, tmp_path):
    ...
    # Verify that a TemporaryDirectory was created.
    assert mock_tmpdir.called, \
        "Expected to see call to `tempfile.TemporaryDirectory(...)`"

    # Find the name of the temporary directory.
    tmpdir_job0 = f"{utils.get_tmpdir_name(mock_tmpdir)}/job-00000"
